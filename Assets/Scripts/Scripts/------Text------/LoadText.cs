...
public class LoadText : MonoBehaviour
{
    ...
    */
    ...
    [SerializeField] private Slider bgm_Slider;//bgm音量調整
    [SerializeField] private Slider se_Slider;//se音量調整
    [SerializeField] private Slider speed_Slider;//会話速度調整
   ...
   float speed = 0;//Autoの速さ
    ...
    void Start()
    {
        ...
        nameText_m.SetActive(false);//男性の名前
        nameText_w.SetActive(false);//女性の名前
        image_women.SetActive(false);//女性の画像
        image_men.SetActive(false);//男性の画像

        menu.SetActive(false);//メニュー
        pagebleak.SetActive(false);//クリックアイコン
